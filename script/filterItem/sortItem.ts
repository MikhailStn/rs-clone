...
export async function filterItem(mas: IPetsitters1[], filter: IFilter) {
  ...
  if (filter.animal) {
    switch (filter.animal) {
      case "Small(up to 6 kg)":
        filter.animal = "cat";
        break;
      case "Large(over 6 kg)":
        filter.animal = "cat";
        break;
      case "Micro(up to 5 kg)":
        filter.animal = "dog";
        break;
      case "Small(6 - 15 kg)":
        filter.animal = "dog";
        break;
      case "Medium(16 - 25 kg)":
        filter.animal = "dog";
        break;
      case "Large(26 - 35 kg)":
        filter.animal = "dog";
        break;
      case "Giant(over 36 kg)":
        filter.animal = "dog";
        break;
    }
  }
  result = result.filter((elem) =>
    elem.petsitterData.services.servicesArr.includes(filter.type)
  );

  if (filter.city.length !== 0) {
    result = result.filter((elem) =>
      elem.city.toLowerCase().includes(filter.city.toLowerCase())
    );
  }
    ...
    case "hotel":
      {
        if (filter.priceMax) {
          result = result.filter(
            (elem) =>
              +elem.petsitterData.services.hotel.price <=
        ...
        if (filter.animal.length) {
          result = result.filter((elem) =>
            elem.petsitterData.services.hotel.animals.includes(
              filter.animal.toLowerCase()
            )
          );
        }
      }
    ...
    case "walking":
      {
        if (filter.priceMax) {
          result = result.filter(
            (elem) =>
              +elem.petsitterData.services.walking.price <=
              Number(filter.priceMax)
          );
        }
        if (filter.priceMin) {
    ...
    case "homevisits":
      {
        if (filter.priceMax) {
          result = result.filter(
            (elem) =>
              +elem.petsitterData.services.homevisits.price <=
              Number(filter.priceMax)
          );
        }
        if (filter.priceMin) {
        ...
        if (filter.animal.length) {
          result = result.filter((elem) =>
            elem.petsitterData.services.homevisits.animals.includes(
              filter.animal.toLowerCase()
            )
          );
        }
  ...
  const nDay1: Date = new Date(
    `${filter.dateFrom.slice(0, 4)}, ${+filter.dateFrom.slice(
      5,
      7
    )}, ${+filter.dateFrom.slice(8)}`
  );
  const nDay2: Date = new Date(
    `${filter.dateTo.slice(0, 4)}, ${+filter.dateTo.slice(
      5,
      7
    )}, ${+filter.dateTo.slice(8)}`
  );
