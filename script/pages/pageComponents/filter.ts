...
export function filterHow(tagParent: HTMLElement): void {
    ...
    setMenuPets(divMenuPets, ['qwe', 'asd', 'zxc']);///////!!!!
      const imgNamePetBox = new Image(); imgNamePetBox.src = "img/paw.svg";
      const inputPetBox = createInputElement("text");///////!!!!
      const img2NamePetBox = new Image(); img2NamePetBox.src = "img/down.svg";
    ...
    const divAddressBox = createHtmlElement("div");
      const imgAddressPetBox = new Image(); imgAddressPetBox.src = "img/geoloc.svg";
    ...
    const imgTimeBox = new Image(); imgTimeBox.src = "img/clock.svg";
    const inputTimeBox = createInputElement("text", "itTime", "itTime");
    const img2TimeBox = new Image(); img2TimeBox.src = "img/down.svg";
...
function setMenuTime(tagParent: HTMLElement): void{
    ...
    if (getMin == 0 || getMin == 30) {getHours += 1; }
    if (getMin > 0 && getMin < 30) {getHours += 1; getMin = 30;}
    if (getMin > 30) {getHours += 2; getMin = 0; }
    ...
    while(getHours < 22){
      ...
      const li = createHtmlElement('li', '', '', `${str}`);
        ...
        tagParent.append(li);
        if(getMin == 0) {getMin = 30; getHours += 0;}
        else if(getMin == 30) {getMin = 0; getHours += 1;}
    }
}
