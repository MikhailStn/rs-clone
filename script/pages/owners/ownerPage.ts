...
const sectionOwnerPerson = createHtmlElement(
    "section",
    "section-owner-person"
  );
...
async function renderOwnerPerson(id: string){
    ...
    const petsitPersonBlock = createHtmlElement('div', 'petsit-person-block-owner');
    ...
    const qwalificCityText = createHtmlElement('h3', 'qualific-city-person-text', '', `Pet owner, ${userInfo.city}`);
    ...
    const contactInfoTitle = createHtmlElement('div', 'contact-info-title','','Contact information');
    const phoneOwner = createHtmlElement('div', 'phone-number-owner','',`Phone number: ${userInfo.phone}`);
