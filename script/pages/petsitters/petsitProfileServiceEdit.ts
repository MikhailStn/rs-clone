  ...
  if (currService == "accommodation") {
    ...
    fetch(`http://localhost:5000/auth/user`, fetchData)
      ...
      .then((data) => {
        ...
        const amountInput = document.querySelector(".input-number-price") as HTMLInputElement;
        amountInput.value = data.petsitterData.services.hotel.price;
      });
    const btnSave = document.querySelector(".btn-save-service-edit") as HTMLButtonElement;
    ...
    const serviceArea = document.querySelector(".input-area-service") as HTMLTextAreaElement;
    let sizeOfDog: string[] = [];
    let ageOfDog: string[] = [];
    let genderOfDog: string[] = [];
    const amount = document.querySelector(".input-number-price") as HTMLInputElement;
    ...
    fetch(`http://localhost:5000/auth/user`, fetchData)
      ...
      .then((data) => {
        ...
        serviceArea.value = data.petsitterData.services.homevisits.serviceArea;
      });
    const btnSave = document.querySelector(".btn-save-service-edit") as HTMLButtonElement;
    btnSave.addEventListener("click", () => {
      const amountInput = document.querySelector(".input-number-price") as HTMLInputElement;
