...
async function createUserBlock(data: OrderPreview) {
  ...
  curUserInfo.role === "OWNER"
    ? (titleUserInfo.textContent = "The service will be provided by:")
    : (titleUserInfo.textContent = "Pet owner:");
  ...
  if (curUserInfo.role === "OWNER") {
    ...
    const sendReviwBlockTitle = createHtmlElement(
      "div",
      "send-review-and-rating",
      "",
      "Here you can evaluate the work of the petsitter and write a review."
    );
    ...
    const btnSaveRateReview = createHtmlElement(
      "button",
      "btn-save-rate-review",
      "btn-save-review",
      "Save Review and Rate"
    );
